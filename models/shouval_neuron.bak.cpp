...
void
ShouvalNeuron::Parameters_::get( DictionaryDatum& d ) const
{
    assert(false);
}

double
ShouvalNeuron::Parameters_::set( const DictionaryDatum& d, Node* node )
{
  assert(false);
}
...
void ShouvalNeuron::pre_run_hook() {
    B_.logger_.init();
...
void ShouvalNeuron::update(nest::Time const & origin, const long from, const long to){
    ...
    for ( long lag = from ; lag < to ; ++lag ) {
        ...
        else if (S_.ode_state[State_::V_m] >= P_.V_th)
        {
            ...
            nest::SpikeEvent se;
            std::cout << "Delivering spike from sender ID: " << this->get_node_id() << std::endl << std::flush;
            nest::kernel().event_delivery_manager.send(*this, se, lag);
            std::cout << "Delivered spike from sender ID: " << this->get_node_id() << std::endl << std::flush;
            std::cout << "Spike Event: " << se.sender_is_valid() << std::endl << std::flush;
